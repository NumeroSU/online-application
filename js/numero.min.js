function onLogin(){_user=$("#github_user").prop("value");_pwd=$("#github_pwd").prop("value");auth=btoa(_user+":"+_pwd);document.cookie="username="+_user;document.cookie="password="+_pwd;$.get({url:"https://api.github.com/users/"+_user+"/repos",headers:{Authorization:"Basic "+auth},success:getRepos})}
function getRepos(b){$("#github_repositories").prop("disabled",!1);$("#github_repositories").html('<option value="" disabled selected>Repository</option>');$("#button_save_modal").prop("disabled",!1);$("#button_load_modal").prop("disabled",!1);for(i=0;i<b.length;i++)$("#github_repositories").append("<option>"+b[i].name+"</option>");repo=b[0].name;getFiles(b[0].name)}
function getFiles(b){$.get({url:"https://api.github.com/repos/"+_user+"/"+b+"/contents",headers:{Authorization:"Basic "+auth},success:function(a){$("#github_files").prop("disabled",!1);$("#github_files").html('<option value="" disabled selected>File</option>');for(i=0;i<a.length;i++)$("#github_files").append("<option>"+a[i].name+"</option>");file=a[0].name;getFileVersions(file)}})}
function getFileVersions(b){$.get({url:"https://api.github.com/repos/"+_user+"/"+repo+"/commits?path="+b,headers:{Authorization:"Basic "+auth},success:function(a){$("#button_load").prop("disabled",!1);$("#github_version").prop("disabled",!1);$("#github_version").html('<option value="" disabled selected>Version</option>');for(i=0;i<a.length;i++)$("#github_version").append("<option sha='"+a[i].commit.tree.sha+"'>"+a[i].commit.author.name+" - "+a[i].commit.message+"</option>");sha=a[0].commit.tree.sha;
console.log(sha);getFileContent(file,sha)}})}function getFileContent(b,a){$.get({url:"https://api.github.com/repos/"+_user+"/"+repo+"/git/trees/"+a,headers:{Authorization:"Basic "+auth},success:function(a){for(i=0;i<a.tree.length;i++)if(a.tree[i].path==b){var c=a.tree[i].sha;_file_sha=c;$.get({url:"https://api.github.com/repos/"+_user+"/"+repo+"/git/blobs/"+c,headers:{Authorization:"Basic "+auth},success:function(a){content=atob(a.content);$("#file_content").html(content)}})}}})};console.log(document.cookie);var _user=null,_pwd=null,auth=null,repo=null,file=null,sha=null,content=null,_file_sha=null,api_ip=$("#serverIP").prop("value");function getCookie(b){b+="=";for(var a=decodeURIComponent(document.cookie).split(";"),e=0;e<a.length;e++){for(var c=a[e];" "==c.charAt(0);)c=c.substring(1);if(0==c.indexOf(b))return c.substring(b.length,c.length)}return""}
...
$("#button_load_default").click(function(){$(this).addClass("is-loading");b4w.require("numero_main").load_default()});$(".modal-close").click(function(){$(".modal").removeClass("is-active")});$(".delete").click(function(){$(".modal").removeClass("is-active")});$("#button_cancel").click(function(){$(".modal").removeClass("is-active")});$("#serverIP").change(function(){api_ip=$(this).prop("value");query_available_models(api_ip)});
